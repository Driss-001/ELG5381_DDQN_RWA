...
class RWAEnv(OpticalNetworkEnv):
    ...
    metadata = {"render_modes": ["service_blocking_rate", "episode_service_blocking_rate","episode_service_accepted"]}
    ...
    def __init__(
        ...
        self.spectrum_wavelengths_allocation = np.full(
        ...
        self.observation_space = gym.spaces.Dict(
            {
                ...
                "service": gym.spaces.Discrete(10),
            }
        )
        self.action_space.seed(self.rand_seed)
        self.observation_space.seed(self.rand_seed)
        self.current_path_load = 0
    ...
    def step(self, action: Sequence[int]):
        ...
        if (
            path < self.k_paths and wavelength < self.num_spectrum_resources
        ):  # if the indices are within the bounds
        ...
        self.current_path_load+=1

        self.topology.graph["services"].append(self.service)

        reward = self.reward()/self.current_path_load
        ...
        info = {
            ...
            "episode_service_accepted":self.episode_services_accepted,

            "path_action_probability": np.sum(self.actions_output, axis=1)
            / np.sum(self.actions_output),
            "wavelength_action_probability": np.sum(self.actions_output, axis=0)
            / np.sum(self.actions_output),
        }
    ...
    def reset(self, *, seed: Optional[int] = None,only_counters=True, **kwargs):
        ...
        self.topology.graph["available_wavelengths"] = np.ones(
    ...
    def render(self, mode="human", close=False):
        ...
        plt.subplot(1, 3, 1)
        plt.pcolor(
            self.spectrum_wavelengths_allocation.transpose(),
            cmap=plt.cm.Greys,
            edgecolors="gray",
            linewidth=0.01,
        )
    ...
    def _provision_path(self, path: Path, wavelength: int):
        ...
        if not self.is_path_free(path, wavelength):
            raise ValueError(
                "Wavelength {} of Path {} is not free".format(
                    wavelength, path.node_list
                )
            )

        for i in range(len(path.node_list) - 1):
            self.topology.graph["available_wavelengths"][
                self.topology[path.node_list[i]][path.node_list[i + 1]]["index"],
                wavelength,
            ] = 0
            self.spectrum_wavelengths_allocation[
                self.topology[path.node_list[i]][path.node_list[i + 1]]["index"],
                wavelength,
...
def get_path_capacity(env: RWAEnv, path: Path) -> int:
    ...
    for wavelength in range(env.num_spectrum_resources):
        ...
        for i in range(len(path.node_list) - 1):
            ...
            if (
                env.topology.graph["available_wavelengths"][
    ...
    for wavelength in range(env.num_spectrum_resources):
        if env.is_path_free(
            env.k_shortest_paths[
                env.service.source, env.service.destination
            ][0],
            wavelength,
        ):
            return (0, wavelength)
...
def shortest_available_path_first_fit(env: RWAEnv) -> Sequence[int]:
    ...
    )  # stores current decision, initilized as "reject"
    for idp, path in enumerate(
        env.topology.graph["ksp"][
            env.service.source, env.service.destination
        ]
    ):
...
class PathOnlyFirstFitAction(gym.ActionWrapper):
    """
    This wrapper enables the agent to only select the path, then selects the spectrum
    ...
    def action(self, action: int) -> Sequence[int]:
        """
        This method transforms an action that only selected the path, into an action
        that selects the path and the first-fit wavelength.
        """
            ...
            for wavelength in range(self.env.num_spectrum_resources):
                if self.env.is_path_free(
                    self.env.topology.graph["ksp"][
                        self.env.service.source,
                        self.env.service.destination,
                    ][action],
                    wavelength,
                ):  # if wavelength is found
